...
$response = $client->request('POST', $server.'/app/term/sync.php',['form_params'=>['op'=>'sync','time'=>'1']]);
$serverJson=$response->getBody();
$serverData = json_decode($serverJson);

$localhost = "http://127.0.0.1:8080";
$response = $client->request('POST', $localhost.'/app/term/sync.php',['form_params'=>['op'=>'sync','time'=>'1']]);

$localData = json_decode($response->getBody());
$localindex=array();
...
//start sync
	if(count($insert_to_server)>0){
		...
		*/
		...
		$response = $client->request('POST', $localhost.'/app/term/sync.php',['form_params'=>['op'=>'get','id'=>"{$idInLocal}"]]);
		$localData=$response->getBody();
		$response = $client->request('POST', $server.'/app/term/sync.php',['form_params'=>['op'=>'insert','data'=>"{$localData}"]]);
	...
	if(count($update_to_server)>0){
		...
		*/
		...
		$response = $client->request('POST', $localhost.'/app/term/sync.php',['form_params'=>['op'=>'get','id'=>"{$idInLocal}"]]);
		$localData=$response->getBody();
		$response = $client->request('POST', $server.'/app/term/sync.php',['form_params'=>['op'=>'update','data'=>"{$localData}"]]);
		echo $response->getBody()."<br>";
	...
	if(count($insert_to_local)>0){
		...
		$response = $client->request('POST', $server.'/app/term/sync.php',['form_params'=>['op'=>'get','id'=>"{$idInServer}"]]);
		$serverData=$response->getBody();
		$response = $client->request('POST', $localhost.'/app/term/sync.php',['form_params'=>['op'=>'insert','data'=>"{$serverData}"]]);
		echo $response->getBody()."<br>";
	...
	if(count($update_to_local)>0){
		...
		$response = $client->request('POST', $server.'/app/term/sync.php',['form_params'=>['op'=>'get','id'=>"{$idInServer}"]]);
		$serverData=$response->getBody();
		$response = $client->request('POST', $localhost.'/app/term/sync.php',['form_params'=>['op'=>'update','data'=>"{$serverData}"]]);
		echo $response->getBody()."<br>";
