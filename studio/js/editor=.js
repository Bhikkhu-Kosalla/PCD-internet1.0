...
function editor_save(){
	if(g_filename.length==""){
		...
		if (inputFileName==null || inputFileName==""){
			ntf_show("Project Name Can not Empty");
			return;
		}
		else{	
...
/*
	...
	if (xmlHttp!=null)
	{
	  ...
	  xmlHttp.send(sendHead+com_xmlToString(gXmlBookData));
...
function menu_file_export_csv(){
	...
	if (xmlHttp!=null)
	  ...
	  var sendHead="filename="+g_filename+"#";
	  xmlHttp.send(sendHead + com_xmlToString(xmlDoc));
	  var_dump(xmlHttp.responseText);
	  }
	else
	  {
...
function menu_file_import_csv(){
	...
	editor_loadDataFromCSV(filename);
}
//import csv begin
var editor_CSVXmlHttp=null;
function editor_loadDataFromCSV(strFileName){
	
	if(window.XMLHttpRequest)
	{// code for IE7, Firefox, Opera, etc.
		editor_CSVXmlHttp=new XMLHttpRequest();
	}
	else if(window.ActiveXObject)
	{// code for IE6, IE5
		editor_CSVXmlHttp=new ActiveXObject("Microsoft.XMLHTTP");
	}
	  
	if (editor_CSVXmlHttp!=null)
	{
		var d=new Date();
		var strLink = "import_csv.php?filename="+strFileName;
		editor_CSVXmlHttp.onreadystatechange=editor_csv_serverResponse;
		editor_CSVXmlHttp.open("GET", strLink, true);
		editor_CSVXmlHttp.send(null);
...
function editor_csv_serverResponse(){
			if (editor_CSVXmlHttp.readyState==4)// 4 = "loaded"
			{
				document.getElementById('id_csv_msg_inner').innerHTML="receve csv data";
			  if (editor_CSVXmlHttp.status==200)
				{// 200 = "OK"
				var xmlText = editor_CSVXmlHttp.responseText;
...
function showModifyWin(sWordId){
	...
	var eWin = document.getElementById("modifywin");
	var eWordInfo = document.getElementById("modify_detaile");
	var eApply = document.getElementById("modify_apply");
	
	var sReal = getNodeText(xAllWord[wid],"real");
	var sParent = getNodeText(xAllWord[wid],"parent");
	var sMeaning = getNodeText(xAllWord[wid],"mean");
	var sOrg = getNodeText(xAllWord[wid],"org");
	var sOm = getNodeText(xAllWord[wid],"om");
	var sCase = getNodeText(xAllWord[wid],"case");
	...
	if(g_useMode=="edit"){
        ...
        tApply += "<input id='checkbox_apply_same' type=\"checkbox\" style=\" width:14px; height:14px; margin-left:10px;\" align=\"left\"/>Update Same Word<span id='same_word_count' >"+sameCount+"</span>";
		tApply += "</span>";
...
function refreshPartMeaningSelect(){
	...
	output += "<div>";
	output+= "<button style='margin-left:auto' onclick=\"copy_part_mean_to_mean()\">";
	output+= "▲";
	output+= "</button>";
	
	output+= "<button style='margin-left:auto' onclick=\"input_org_switch('input_org_select','input_om')\">";
				...
				for(iWord=0;iWord<xWord.length;iWord++){
					if(getNodeText(xWord[iWord],"id")==parentId){
						...
						for(iNewWord=0;iNewWord<sSubPali.length;iNewWord++){
							...
							var newGUID=parentId+"_"+iNewWord;
...
function editor_openProject(strFileName){
	...
	if (editor_openProjectXmlHttp!=null){
		var d=new Date();
		var strLink ="";
		strLink = "project_load.php?filename="+strFileName;
		if(strLink.length>0){
...
//remove the 'wb' in string head
	...
	if(gRelationSelectWordBegin ){
		...
		let eWord=document.getElementById("wb"+_curr_mouse_enter_wordid);
			...
			for(var x=0;x<xAllWord.length;x++){
				...
				for(var i=0;i<dict_data.length;i++){
					if(dict_data[i].book==book && dict_data[i].paragraph==para && dict_data[i].num==num ){
						...
						//updataWordBodyByElement(xAllWord[x]);
						break;
					}
				}
			}
