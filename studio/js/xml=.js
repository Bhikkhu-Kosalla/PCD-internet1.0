...
function modifyApply(sWordId,update_user_dict){
	...
	if(sPali!=oldPali){
		...
		str_msg_data += "\"pali\":\""+sPali+"\",";
	}
	setNodeAttr(x[wordIndex],"bmt","pali",wordCurrStatus);
	...
	if(sReal != oldPali){
		...
		str_msg_data += "\"real\":\""+sReal+"\",";
	}
	setNodeAttr(x[wordIndex],"bmt","real",wordCurrStatus);
	...
	if(sMeaning != oldMean){
		...
		str_msg_data += "\"mean\":\""+sMeaning+"\",";
	}	
	setNodeAttr(x[wordIndex],"bmt","mean",wordCurrStatus);
	...
	if(sParent != oldParent){
		...
		str_msg_data += "\"parent\":\""+sParent+"\",";
	}
	setNodeAttr(x[wordIndex],"bmt","parent",wordCurrStatus);
	
	let sOrg = document.getElementById("input_org").value;
	let oldOrg = getNodeText(x[wordIndex],"org");
	if(oldOrg != sOrg){
		setNodeText(x[wordIndex],"org",sOrg);	
		str_msg_data += "\"org\":\""+sOrg+"\",";
	}
	setNodeAttr(x[wordIndex],"bmt","org",wordCurrStatus);
	...
	setNodeAttr(x[wordIndex],"bmt","om",wordCurrStatus);
	...
	if(oldCase != sCase){
		...
		str_msg_data += "\"case\":\""+sCase+"\",";
	}
	setNodeAttr(x[wordIndex],"bmt","case",wordCurrStatus);
	...
	if(oldLock != bLocked){
		...
		str_msg_data += "\"lock\":\""+bLocked+"\",";
	}	
	setNodeAttr(x[wordIndex],"bmt","lock",wordCurrStatus);
	...
	if(oldBookMarkColor!=g_currBookMarkColor){
		...
		str_msg_data += "\"bmc\":\""+g_currBookMarkColor+"\",";
		g_currBookMarkColor="";
	}
	...
	if(oldRalation!=sRalation){
		...
		str_msg_data += "\"rela\":\""+encodeURI(sRalation)+"\",";
	...
	
	{
		...
		str_msg_data += "\"status\":\""+wordCurrStatus+"\"}";
	}
	...
	if(doc_info.sendmsg){
		...
		msg_push(1,str_msg_data,msg_doc_id,d.getTime(),book,paragraph);	
	}
			...
			if(sWordId!=iSearch){
				...
				if(xmlNotePali==sPaliReal ){
					if(getNodeText(x[iSearch],"lock")!="true"){
						...
						debugOutput("updata:"+iSearch+"<br />",0);
