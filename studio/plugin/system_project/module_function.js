...
function editor_project_translate_addnew(bWithText){
	...
	newTranslateLanguage=document.getElementById("id_project_translate_new_language").value;
	newTranslateAuthor=document.getElementById("id_project_translate_new_author").value;
	
	var album_guid = new Array();
	for(var iPar=0;iPar<gArrayDocParagraph.length;iPar++){
		BookId=gArrayDocParagraph[iPar].book;
		album_guid[BookId.toString()]=com_guid();
	}
	for(var iPar=0;iPar<gArrayDocParagraph.length;iPar++){
		var BookId=gArrayDocParagraph[iPar].book;
		var ParId=gArrayDocParagraph[iPar].paragraph;
		var iLevel=gArrayDocParagraph[iPar].level;
		strXml += "<block>"
		strXml += "<info>";
		strXml += "<album_id>-1</album_id>";
		strXml += "<album_guid>"+album_guid[BookId.toString()]+"</album_guid>";
		strXml += "<id>"+com_guid()+"</id>";
					...
					if(xWord.length>0){
						...
						for(var iWord=0;iWord<xWord.length;iWord++){
							...
							if(wEnter==1){
								...
								strXml+="<end>"+iend+"</end>";
								strXml+="<text>"+gui_string_editor_project[0]+"</text>";
								strXml+="</sen>";
								iend++;
								ibegin=iend;
							}
